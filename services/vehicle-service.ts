...
const listVehicles = async (query: any) => {
    try {
...
const deleteVehicle = async (id: string) => {
    try {
        const findout = await prisma.vehicle.findFirst({
            where: {
                id: id
            },
            include: {
        ...
        if (!_.isEmpty(findout) && _.isEmpty(findout?.vehicleBooking)) {
            return await prisma.vehicle.delete({
                where: { id: id }
            })
        } else {
            successAndErrors.throwCustomMessage("This vehicle contain booking, Please delete booking before to delete Vehicle, Vehicle")
        }

    } catch (error:any) {
        throw successAndErrors.deleteFailure(error?.customMessage ?? 'Vehicle')
    }
}
